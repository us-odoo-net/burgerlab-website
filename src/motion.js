...
// ─── Scrub tuning (edit here) ───────────────────────────────────────────────
// Minimum seek step in seconds: higher = fewer seeks (lighter on the main
// thread, smoother touch scrolling) but coarser scrub. 1 frame @24fps ≈ 0.042.
const SEEK_EPSILON_DESKTOP = 0.035 // ~0.8 frame — maximum detail
const SEEK_EPSILON_MOBILE = 0.055 // ~1.3 frames — relieves mid-range phones
// Per-mode posters = frame 0 of each film (generated by scripts/encode-films.sh)
// so the poster→film handoff is invisible (identical framing). The films'
// FRAMING itself (how "pulled back" the burger looks) is tuned in
// scripts/encode-films.sh via CANVAS_* / FILM_SCALE_*.
const POSTER_DESKTOP = 'img/poster-desktop.webp'
const POSTER_MOBILE = 'img/poster-mobile.webp'
// ────────────────────────────────────────────────────────────────────────────
...
function boot() {
  ...
  if (bgVideo && !dataSaver) {
    ...
    // Matching poster: frame 0 of the mode's own film, so unveiling the page
    // never "jumps framing" (the reported mobile hard-cut on load).
    const wantedPoster = isMobile ? POSTER_MOBILE : POSTER_DESKTOP
    if (!(bgVideo.poster || '').endsWith(wantedPoster)) {
      bgVideo.poster = wantedPoster
    }
  }
  if (dataSaver) {
  ...
  if (bgVideo && !dataSaver) {
    ...
    const seekEpsilon = isMobile ? SEEK_EPSILON_MOBILE : SEEK_EPSILON_DESKTOP
    let disposed = false
    const doVideoUpdate = () => {
      if (disposed || !bgVideo.duration) return
      ...
      if (t === null) return
      if (Math.abs(t - lastVideoT) > seekEpsilon) {
